...
describe("agent:// protocol", () => {
	...
	it("describes missing finalized output for idle agents without calling them live", async () => {
		await withTempDir(async dir => {
			const artifactsDir = path.join(dir, "artifacts");
			await fs.mkdir(artifactsDir, { recursive: true });
			await Bun.write(path.join(artifactsDir, "planner.md"), "planner output");
			...
			expect(error).toBeInstanceOf(Error);
			expect(error?.message).toContain("Finalized output not found: buildBranch");
			expect(error?.message).toContain("Agent buildBranch is idle");
			expect(error?.message).toContain("Read the transcript with history://buildBranch");
			expect(error?.message).not.toContain("live transcripts");
			expect(error?.message).toContain("Available finalized outputs: planner");
		});
	});
});
