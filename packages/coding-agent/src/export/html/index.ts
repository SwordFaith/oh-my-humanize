...
export function buildSessionData(sm: SessionManager, state?: AgentState): SessionData {
	...
	return {
		header: sm.getHeader(),
		entries: redactSessionEntriesForHtmlExport(rawEntries),
		leafId: sm.getLeafId(),
		systemPrompt: state?.systemPrompt.join("\n\n"),
		tools: state?.tools?.map(t => ({ name: t.name, description: t.description })),
		workflowInspections: buildWorkflowInspections(rawEntries),
		workflowLifecycleInspections: buildWorkflowLifecycleInspections(rawEntries),
	};
}
...
async function collectSubSessionsFromDir(
	...
	for (const name of names) {
		...
		if (fileEntries.length > 0) {
			...
			const entries = redactSessionEntriesForHtmlExport(
				fileEntries.filter((e): e is SessionEntry => e.type !== "session"),
			);
			out[key] = {
				agentId,
				parent: parentKey,
