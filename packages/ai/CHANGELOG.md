...
- Fixed OpenAI Responses-compatible SSE streams that emit comment-only keepalive frames after a named `event:` line, which previously surfaced as a JSON parse transport error instead of being ignored per SSE semantics.
