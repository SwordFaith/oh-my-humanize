...
| `OMHFLOW_DIR`         | Path-list of external `.omhflow` flow library roots. `omp workflow install` writes to the first entry; named workflow lookup treats bundled and external flow artifacts as peers and rejects ambiguous same-name matches. Defaults to `~/.omp/flows` when unset |
